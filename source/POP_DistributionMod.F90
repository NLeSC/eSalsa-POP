...
!
! !REVISION HISTORY:
!  SVN:$Id: POP_DistributionMod.F90 141 2009-09-21 20:50:15Z maltrud $
...
! !INTERFACE:
 ...
 function POP_DistributionCreate(distrbMethod, numProcs, workPerBlock, &
                                 distribution_file, errorCode) result(newDistrb)
...
! !INPUT PARAMETERS:
   ...
   character (*), intent(in) :: &
      distribution_file     ! input file for distribution
...
!----------------------------------------------------------------------
   ...
   case(POP_distribMethodRake)
      ...
      ! KLUDGE: probably should do something better here 
      !------------------------------------------------
!JASON      allocate(work_per_block(size(workPerBlock)))
!      maxWork = MAXVAL(workPerBlock)
!      work_per_block = maxWork
...
!      deallocate(work_per_block)
   ...
   case(POP_distribMethodPredefined)

      newDistrb = POP_DistributionCreatePredefined(numProcs, &
                                             distribution_file, errorCode)

      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
               'POP_DistributionCreate: error in Predefined create')
         return
      endif
...
!JASON Added check - may not be allocated with dynamic work distribution!
...
!JASON ADDED!!
   nullify(newDistrb%blockGlobalID)
! END JASON
...
#ifdef JASON_PRINT
!   write(*,*) 'JASON:    INITIAL CART DIST'
!   write(*,*) 'JASON:        numProcs = ', dist%numProcs
!   write(*,*) 'JASON:        communicator = ', dist%communicator
!   write(*,*) 'JASON:        numLocalBlocks = ', dist%numLocalBlocks
!   write(*,*) 'JASON:        blockLocation = ', dist%blockLocation
!   write(*,*) 'JASON:        blockLocalID = ', dist%blockLocalID
!   write(*,*) 'JASON:        blockGlobalID = ', dist%blockGlobalID
#endif
...
#ifdef JASON_PRINT
 !  write(*,*) 'JASON:    INITIAL CART DIST after X RAKE'
 !  write(*,*) 'JASON:        numProcs = ', dist%numProcs
 !  write(*,*) 'JASON:        communicator = ', dist%communicator
 !  write(*,*) 'JASON:        numLocalBlocks = ', dist%numLocalBlocks
 !  write(*,*) 'JASON:        blockLocation = ', dist%blockLocation
...
#ifdef JASON_PRINT
 !  write(*,*) 'JASON:    INITIAL CART DIST after Y RAKE'
 !  write(*,*) 'JASON:        numProcs = ', dist%numProcs
 !  write(*,*) 'JASON:        communicator = ', dist%communicator
 !  write(*,*) 'JASON:        numLocalBlocks = ', dist%numLocalBlocks
 !  write(*,*) 'JASON:        blockLocation = ', dist%blockLocation
...
#ifdef JASON_PRINT
   write(*,*) 'JASON:    NEW DIST procTmp ', procTmp
#endif
...
#ifdef JASON_PRINT
   write(*,*) 'JASON:    NEW DIST block local ID ', newDistrb%blockLocalID
#endif
...
#ifdef JASON_PRINT
   write(*,*) 'JASON:    ALLOC GLOBALID ', newDistrb%numLocalBlocks
#endif
...
#ifdef JASON_PRINT
         write(*,*) 'JASON:    ASSIGN GLOBALID ', localBlock, n
#endif
...
function POP_DistributionCreatePredefined(numProcs, &
                          distribution_file, errorCode) result(newDistrb)

! !DESCRIPTION:
!  This function reads a distribution of blocks across processors
!  from a file.
!
! !REVISION HISTORY:
!  same as module
...
#ifdef JASON_PRINT
   write(*,*) 'JASON: POP loading predefined distribution: ', distribution_file
#endif
...
!----------------------------------------------------------------------
   ...
   if (istat > 0) then
      call POP_ErrorSet(errorCode, &
         'POP_DistributionCreatePredefined: error allocating blockLocation')
      return
   endif
...
!    INQUIRE(iolength=reclength) newDistrb%blockLocation
...
#ifdef JASON_PRINT
    write(*,*) 'POP opening distribution: reclength = ', reclength
#endif
    ...
    if (ioerr /= 0) then
        call POP_ErrorSet(errorCode, &
         'POP_DistributionCreatePredefined: error opening distribution_file')
        return
    endif
...
!                               'Blocks per core exceeds POP_maxBlocksTropic')
    ...
    if (ioerr /= 0) then
          call POP_ErrorSet(errorCode, &
         'POP_DistributionCreatePredefined: error reading distribution_file')
        return
    endif

#ifdef JASON_PRINT
    write(*,*) 'JASON: POP bcast distribution'
#endif

    call POP_Broadcast(newDistrb%blockLocation, POP_masterTask, errorCode)
...
!----------------------------------------------------------------------
...
#ifdef JASON_PRINT
    write(*,*) 'JASON: POP determine local blocks'
#endif
    ...
    do i=1,POP_numBlocks
         processor = newDistrb%blockLocation(i)

         if (processor /= 0) then
            proc_tmp(processor) = proc_tmp(processor) + 1
            newDistrb%blockLocalID(i) = proc_tmp(processor)
         endif
    end do
...
#ifdef JASON_PRINT
    write(*,*) 'JASON: POP determine local blocks 2'
#endif
    ...
    if (newDistrb%numLocalBlocks > 0) then
       ...
       if (istat > 0) then
          call POP_ErrorSet(errorCode, &
          'POP_DistributionCreatePredefined: error allocating blockGlobalID')
          return
       endif
       ...
       do i=1,POP_numBlocks
            ...
            if (processor == POP_myTask + 1) then
               newDistrb%blockGlobalID(localID) = i
               localID = localID + 1
            endif
       end do
...
#ifdef JASON_PRINT
   write(*,*) 'JASON: POP predefined distribution done'
#endif

!----------------------------------------------------------------------
!EOC

end function POP_DistributionCreatePredefined
