...
!  tracers.
!
!  NOTE: Merge with CCSM-POP removed option of using GM bolus velocity
!        (M Maltrud, June 2, 2009)
!
! !REVISION HISTORY:
!  SVN:$Id: advection.F90 12674 2008-10-31 22:21:32Z njn01 $
...
! !INPUT PARAMETERS:
   ...
   character(len=POP_CharLength), intent(in) :: &
      tadvect_ctype      ! character string for tracer advect choice

! !OUTPUT PARAMETERS:

   integer(kind=POP_i4) :: tadvect_ctype_to_tadvect_itype
...
!-----------------------------------------------------------------------
   ...
   logical (POP_Logical), dimension(nt) :: &
...
! !INPUT PARAMETERS:
   ...
   logical (POP_logical), dimension(nt), intent(in) :: &
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   real (POP_r8), dimension(nx_block,ny_block,nt), intent(inout) :: &
      LTK                 ! returned as L(T) for nth tracer at level k
