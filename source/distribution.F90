...
!
! !REVISION HISTORY:
! CVS:$Id: distribution.F90 12674 2008-10-31 22:21:32Z njn01 $
! CVS:$Name: $
...
! !INTERFACE:
 ...
 function create_distribution(dist_type, nprocs, workPerBlock, distribution_file)
...
! deallocate(work_per_block)
   ...
   case('predefined')
      ...
      create_distribution = create_distrb_predefined(nprocs, distribution_file)
...
#ifdef JASON_PRINT
   write(*,*) 'JASON: cart dist proc: ', dist%proc
   write(*,*) 'JASON: cart dist local_block: ', dist%local_block
#endif
...
#ifdef JASON_PRINT
   print *, 'JASON: Reducing blocks from ', nblocks, ' to ', ii
#endif
...
!maltrud merge balanced distribution function is different
...
#ifdef JASON_PRINT
   write(*,*) 'JASON: cart balanced proc: ', dist%proc
   write(*,*) 'JASON: cart balanced local_block: ', dist%local_block
#endif
...
#ifdef JASON_PRINT
         write (*,*) 'JASON valid found proc config ', iguess, jguess
#endif
...
#ifdef JASON_PRINT
   	    write (*,*) 'JASON valid found distribution proc=', iguess, jguess, 'block=', nblocks_x, nblocks_y
#endif
            exit proc_loop
...
#ifdef JASON_PRINT
            write (*,*) 'JASON valid found distribution (t) proc=', jguess, iguess, 'block=', nblocks_x, nblocks_y
#endif
            exit proc_loop
...
function create_distrb_predefined(nprocs, distribution_file)

! !DESCRIPTION:
! This function reads the distribution of blocks across processors
! from an input file.
!
! !REVISION HISTORY:
! v0.1 Jason Maassen 23/05/2012
...
! !OUTPUT PARAMETERS:
   ...
   type (distrb) :: &
      create_distrb_predefined ! resulting structure describing
                                ! distribution of blocks
!EOP
!BOC
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
!
! allocate space for decomposition, and init to 0
!
!----------------------------------------------------------------------
...
!    INQUIRE(iolength=reclength) dist%proc
...
#ifdef JASON_PRINT
    write(*,*) 'JASON: opening distribution file ', reclength
#endif
    ...
    if (ioerr /= 0) call exit_POP(sigAbort, &
                              'Error opening distribution_file')
...
!        read(nu, rec=1, iostat=ioerr) dist%proc
    ...
    if (ioerr /= 0) call exit_POP(sigAbort, &
                               'Error reading distribution_file')

#ifdef JASON_PRINT
    write(*,*) 'JASON: bcast distribution'
#endif

    call broadcast_array(dist%proc, master_task)
...
#ifdef JASON_PRINT
    write(*,*) 'JASON: determining local blocks'
#endif
...
#ifdef JASON_PRINT
            write(*,*) 'JASON: proc_tmp(', pid, ')= ', proc_tmp(pid)
#endif
...
#ifdef JASON_PRINT
      write(*,*) 'JASON: minval(proc_tmp)=', minval(proc_tmp)
#endif
...
#ifdef JASON_PRINT
    write(*,*) 'JASON: predefined distribution DONE'
#endif

!----------------------------------------------------------------------

   create_distrb_predefined = dist ! return the result

!----------------------------------------------------------------------
!EOC

end function create_distrb_predefined
