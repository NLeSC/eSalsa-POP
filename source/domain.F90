...
! !PUBLIC DATA MEMBERS:
   ...
   integer (POP_i4), dimension(:), pointer, public :: &
...
!----------------------------------------------------------------------
   ...
   distribution_file = 'unknown_distribution_file'
...
subroutine init_domain_blocks(errorCode)
   ...
   case ('predefined','Predefined','PREDEFINED')
      clinicDistributionMethod = POP_distribMethodPredefined
   case default
   ...
   case ('predefined','Predefined','PREDEFINED')
      tropicDistributionMethod = POP_distribMethodPredefined
   case default
      call POP_ErrorSet(errorCode, &
...
!----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
     ...
     write(stdout,'(a31,a10)') '  Distribution file: ', &
                                 trim(distribution_file)
...
! !INTERFACE:
 ...
 subroutine init_domain_distribution(KMTG, errorCode)
...
! !INPUT PARAMETERS:
   ...
   integer (POP_i4), dimension(nx_global,ny_global), intent(in) :: &
...
!----------------------------------------------------------------------
   ...
   character (POP_charLength) :: outstring
...
!JASON If _HIRES is not set, all points are computed, even land points.... WHY?
!maltrud merge why is HIRES different?
#ifdef _HIRES
...
#ifdef JASON_PRINT
      if(my_task == master_task) then
         write(*,*) 'JASON block ', n, ' has ', nocn(n), 'work'
      endif
#endif
...
!----------------------------------------------------------------------
   ...
   distrb_tropic = create_distribution(tropic_distribution_type, &
                                       nprocs_tropic, work_per_block, distribution_file)

   distrb_clinic = create_distribution(clinic_distribution_type, &
                                       nprocs_clinic, work_per_block, distribution_file)

   POP_distrbClinic = POP_DistributionCreate(clinicDistributionMethod, &
                         nprocs_clinic, work_per_block, distribution_file, errorCode)
   ...
   POP_distrbTropic = POP_DistributionCreate(tropicDistributionMethod, &
                         nprocs_tropic, work_per_block, distribution_file, errorCode)
...
#ifdef JASON_PRINT
   write(*,*) 'JASON: distrb_tropic'
   write(*,*) 'JASON:    nprocs = ', distrb_tropic%nprocs
   write(*,*) 'JASON:    communicator = ', distrb_tropic%communicator
   write(*,*) 'JASON:    proc = ', distrb_tropic%proc
   write(*,*) 'JASON:    local_block = ', distrb_tropic%local_block
   write(*,*) 'JASON: '
   write(*,*) 'JASON: distrb_clinic'
   write(*,*) 'JASON:    nprocs = ', distrb_clinic%nprocs
   write(*,*) 'JASON:    communicator = ', distrb_clinic%communicator
   write(*,*) 'JASON:    proc = ', distrb_clinic%proc
   write(*,*) 'JASON:    local_block = ', distrb_clinic%local_block
   write(*,*) 'JASON: '
   write(*,*) 'JASON: POP_distrbClinic'
   write(*,*) 'JASON:    numProcs = ', POP_distrbClinic%numProcs
   write(*,*) 'JASON:    communicator = ', POP_distrbClinic%communicator
   write(*,*) 'JASON:    numLocalBlocks = ', POP_distrbClinic%numLocalBlocks
   write(*,*) 'JASON:    blockLocation = ', POP_distrbClinic%blockLocation
   write(*,*) 'JASON:    blockLocalID = ', POP_distrbClinic%blockLocalID
   write(*,*) 'JASON:    blockGlobalID = ', POP_distrbClinic%blockGlobalID
   write(*,*) 'JASON: '
   write(*,*) 'JASON: POP_distrbTropic'
   write(*,*) 'JASON:    numProcs = ', POP_distrbTropic%numProcs
   write(*,*) 'JASON:    communicator = ', POP_distrbTropic%communicator
   write(*,*) 'JASON:    numLocalBlocks = ', POP_distrbTropic%numLocalBlocks
   write(*,*) 'JASON:    blockLocation = ', POP_distrbTropic%blockLocation
   write(*,*) 'JASON:    blockLocalID = ', POP_distrbTropic%blockLocalID
   write(*,*) 'JASON:    blockGlobalID = ', POP_distrbTropic%blockGlobalID
#endif 
