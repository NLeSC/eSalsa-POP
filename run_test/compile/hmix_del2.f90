...
!-----------------------------------------------------------------------
   ...
   if (lauto_hmixu) then
      ...
      am = 1.0e7_POP_r8*(720.0_POP_r8/float(nx_global))
      if (my_task == master_task) then
         write(stdout,'(a44)') &
   ...
   if (lvariable_hmixu) then
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'init_del2u: error updating AMF halo')
         return
      endif
...
!-----------------------------------------------------------------------
   ...
   if (lauto_hmixt) then
      ...
      ! scale to 1e7 at 1/2 degree
      ah = 1.0e7_POP_r8*(720.0_POP_r8/float(nx_global))
      if (my_task == master_task) then
         write(stdout,'(a46)') &
...
!-----------------------------------------------------------------------
   ...
   if (lvariable_hmixt) then
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'init_del2u: error updating AHF halo')
         return
      endif
...
! \nonumber \! & &- u\left(\delta_x k_x + \delta_y k_y +
...
! \nonumber \! & &- v\left(\delta_x k_x + \delta_y k_y +

! 2(k_x^2 + k_y^2)\right) \nonumber \! & &+ 2k_y \delta_x u - 2k_x \delta_y u
...
! !INTERFACE:
 ...
 subroutine hdifft_del2(k,HDTK,TMIX,this_block, errorCode)
