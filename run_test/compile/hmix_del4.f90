...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
      ...
      if (lauto_hmix) then
         am = -0.6e20_POP_r8*(1280.0_POP_r8/float(nx_global))
         write(stdout,'(a44)') &
   ...
   if (lvariable_hmixu) then
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'init_del4u: error updating AMF halo')
         return
      endif
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
      ...
      if (lauto_hmix) then
         ah = -0.2e20_POP_r8*(1280.0_POP_r8/float(nx_global))
         write(stdout,'(a46)') &
...
!-----------------------------------------------------------------------
   ...
   if (lvariable_hmixt) then
      ...
      if (errorCode /= POP_Success) then
         call POP_ErrorSet(errorCode, &
            'init_del4t: error updating AHF halo')
         return
      endif
...
! !INTERFACE:
 ...
 subroutine hdiffu_del4(k,HDUK,HDVK,UMIXK,VMIXK,this_block, errorCode)
...
! + \Delta_y\delta_y[\Delta_x\delta_y(u)]/AREA \! & & - u*[dxkx - dyky + 2(k_x^2 + k_y^2)]

! + 2hy\delta_x(v) - 2k_x\delta_y(v) \nonumber \! \nabla^2(v) & = & \Delta_x\delta_x[\Delta_y\delta_x(v)]/AREA

! + \Delta_y\delta_y[\Delta_x\delta_y(v)]/AREA \! & & - v*[dxkx - dyky + 2(k_x^2 + k_y^2)]
...
! !INTERFACE:
 ...
 subroutine hdifft_del4(k,HDTK,TMIX,this_block, errorCode)
