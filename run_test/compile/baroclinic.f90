...
! Momentum equations:
! \begin{eqnarray}
! (u'-u^{n-1})/(2 \Delta t) - f*\alpha*(v'-v^{n-1}) &=& F_x \! (v'-v^{n-1})/(2 \Delta t) + f*\alpha*(u'-u^{n-1}) &=& F_y

! \end{eqnarray}
! \begin{eqnarray}
! \tilde{u}' &=& u' - {1\over{H_U}}\sum_{k=1}^{km}dz_k u_k' \!     \tilde{v}' &=& v' - {1\over{H_U}}\sum_{k=1}^{km}dz_k v_k'
...
! \begin{eqnarray}
! F_x &=& -L(u) + fv - \nabla p + D_H(u^{n-1}) + D_V(u^{n-1}) \! F_y &=& -L(v) - fu - \nabla p + D_H(v^{n-1}) + D_V(v^{n-1})
...
! \begin{eqnarray}
! +fv &\rightarrow& +f(\gamma v + (1-\gamma)v^{n-1}) \! -fu &\rightarrow& -f(\gamma u + (1-\gamma)u^{n-1})

! \end{eqnarray}
! on leapfrog timesteps and
! \begin{eqnarray}
! +fv &\rightarrow& +fv^{n-1} \! -fu &\rightarrow& -fu^{n-1}
