...
!
! !REVISION HISTORY:
! SVN:$Id: barotropic.F90 14725 2009-03-04 22:50:06Z njn01 $
...
! use distribution, only:
   ...
   use prognostic, only: GRADPX, GRADPY, UBTROP, VBTROP, &
...
!-----------------------------------------------------------------------
   ...
   if (sfc_layer_type == sfc_layer_varthick) then
      ...
      allocate ( CHECKER(nx_block,ny_block,POP_maxBlocksClinic), &
                   CONSTNT(nx_block,ny_block,POP_maxBlocksClinic), &
                CHECK_AREA(nx_block,ny_block,POP_maxBlocksClinic), &
                CONST_AREA(nx_block,ny_block,POP_maxBlocksClinic))
